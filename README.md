...
반영을 시작한 뒤에는 완료된 파일 카드의 `결과 보기`에서 행별 기준상품, 요청한 추가구성상품, Cafe24 응답 추가구성상품, POST/PUT, HTTP 상태, 성공·실패 메시지를 확인할 수 있습니다. `결과 CSV 다운로드`는 같은 내용을 내려받습니다. 이 기록은 브라우저 IndexedDB에 보관하며, access token·refresh token 원문과 Cafe24 원문 응답 전체는 포함하지 않습니다.

완료 기록은 카드별로 삭제할 수 있고, 반영 작업이 없을 때는 `완료 기록 전체 삭제`로 모두 비울 수 있습니다. 삭제는 브라우저의 IndexedDB에서만 이루어지며 Cafe24 상품 설정은 변경하지 않습니다.
